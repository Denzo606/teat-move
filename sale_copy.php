...
<?php
if (isset($_GET['id'])) :
    $sale = $conn->query("SELECT * FROM sale where id = {$_GET['id']}");
    ...
    $items = $conn->query("SELECT s.*,p.pro_name p.price FROM saleproduct s INNER JOIN products p ON p.id = s.pro_id WHERE s.sale_id  = $id ");
endif;
?>
                        ...
                        <div class=" row justify-content-start align-items-center" id="cat-list">
                            ...
                            ?>
                                <div class="mx-3 cat-item" data-id='<?php echo $row['id'] ?>'>
                                    <button class="btn btn-primary"><?php echo ucwords($row['cate_name']) ?></button>
                                </div>
                            <?php endwhile; ?>
                        </div>
                                ...
                                <div class="col-md-2 mb-2">
                                    <div class="prod-item text-center " data-json='<?php echo json_encode($row) ?>' data-category-id="<?php echo $row['category_id'] ?>">
                                        ...
                                        if (file_exists($image_path) && !empty($row['image'])) {
                                            ...
                                            echo '<img src="' . $image_path . '" alt="User Image" class="rounded" width="100%">';
                                        } else {
                            ...
                            <table class="table bg-light mb-5">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center btn-lg justify-content-center">
                                                        <span class=" btn-minus"><b> </b></span>
                                                        <input type="number" class="form-control" name="qty[]" id="" value="<?php echo $row['qty'] ?>">
                                                        <span class="btn-plus btn-sm"><b></b></span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <input type="hidden" name="items_id[]" id="" value="<?php echo $row['id'] ?>">
                                                    <input type="hidden" name="pro_id[]" id="" value="<?php echo $row['pro_id'] ?>"><?php echo ucwords($row['pro_name']) ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    ...
                    <div class="form-group">
                        <label for="">Amount Tendered USD</label>
                        <input type="text" class="form-control text-right" id="tendered" value=" " autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label for="">Amount Tendered KH</label>
                        <input type="text" class="form-control text-right" id="tendered_kh" value=" " autocomplete="off">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="">Change RIEL</label>
                        <input type="text" class="form-control text-right" id="changekh" value="0.00" readonly="">
                    </div>
                    <div class="form-group">
                        <label for="">Exchange rate</label>
                        <input type="text" class="form-control text-right" id="change" value="4150 រៀល" readonly="">
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#prod-list .prod-item').click(function() {
        ...
        tr.append('<td><div class="d-flex align-items-center btn-group"><span class="btn-minus"><b>-</b></span><input type="text"  name="qty[]" value="1"><span class="btn-plus"><b>+</b></span></div></td>');




        tr.append('<td><input type="hidden" name="item_id[]" id="" value=""><input type="hidden" name="pro_id[]" id="" value="' + data.id + '">' + data.pro_name + ' <small class="psmall">($' + (parseFloat(data.price).toLocaleString("en-US", {
            style: 'decimal',
    ...
    function calc() {
        ...
        function updateTotal() {
            ...
            $('[name="qty[]"]').each(function(index) {
                ...
                var price = parseFloat($('[name="price[]"]').eq(index).val());
                var amount = qty * price;
                $('[name="amount[]"]').eq(index).val(amount);
                $('.amount').eq(index).text('$' + amount.toLocaleString("en-US", {
                    style: 'decimal',
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }));
    ...
    $("#pay").click(function() {
        ...
        $('#apayable').val(new Intl.NumberFormat('en-US', {
            style: 'currency',
            currency: 'USD'
    ...
    $('#tendered').keyup('input', function(e) {
        ...
        $('#change').val(new Intl.NumberFormat('en-US', {
            style: 'currency',
            currency: 'USD'
        }).format(parseFloat(change)));
        var change = parseFloat(tend) - parseFloat(amount)
        var kh = parseFloat(change) * 4150;
        $('#changekh').val(new Intl.NumberFormat('km-KH', {
            style: 'currency',
            currency: 'KHR'
        }).format(parseFloat(kh)));

    })
    // change to khmer
    $('#tendered_kh').keyup('input', function(e) {
        if (e.which == 13) {
            $('#manage-order').submit();
            return false;
        }
        ...
        tend = tend > 0 ? tend : 0;
        var amount = $('[name="total_amount"]').val()
        var currency_usd = amount * 4150

        var change = parseFloat(tend) - parseFloat(currency_usd)
        var change_to_usd = parseFloat(change) / parseFloat(4150)
        $('#change').val(new Intl.NumberFormat('en-US', {
            style: 'currency',
            currency: 'USD'
        }).format(parseFloat(change_to_usd)));
        // var change = parseFloat(tend) - parseFloat(amount)
        // var kh = parseFloat(change) * 4150;
        $('#changekh').val(new Intl.NumberFormat('km-KH', {
            style: 'currency',
