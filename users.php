	...
	<div class="col-lg-12">
		<div class="card">
			<div class="card-header khmer-text d-flex justify-content-between align-items-center">
				<b>បញ្ជីអ្នកប្រើប្រាស់</b>
				<a href="javascript:void(0)" class="new_user" id="new_user" data-toggle="tooltip" title="អ្នកប្រើប្រាស់ថ្មី">
							...
							<tr>
								...
								<td>
									...
									if (file_exists($image_path) && !empty($row['image'])) {
										...
										echo '<img src="' . $image_path . '" alt="User Image" style="max-width: 60px; max-height: 60px ;">';
									} else {
								...
								<td>
									<center>
										...
										<a href="javascript:void(0)" class="edit_user" data-id='<?php echo $row['id']; ?>' data-toggle="tooltip" title="កែប្រែ">
											<i class="fa fa-edit"></i>
										</a>

										<?php if (isset($_SESSION['login_type']) && $_SESSION['login_type'] == 1) : // Admin user 
										?>
											<?php if ($row['id'] != $_SESSION['login_id']) : // If not viewing own account 
											?>
												<!-- Delete Icon with Tooltip -->
												<a href="javascript:void(0)" class="delete_user" data-id='<?php echo $row['id']; ?>' data-toggle="tooltip" title="លុប">
													<i class="fa fa-trash-alt"></i>
												</a>
											<?php else : // Viewing own account 
											?>
												<!-- Disabled Delete Icon with Tooltip -->
												<a href="javascript:void(0)" class="delete_user disabled" data-id='<?php echo $row['id']; ?>' data-toggle="tooltip" title="មិនអាចលុបបាន">
													<i class="fa fa-trash-alt"></i>
												</a>
											<?php endif; ?>
										<?php endif; ?>
...
<script>
	$(function() {
		...
		$('#new_user').click(function() {
			uni_modal('អ្នកប្រើប្រាស់ថ្មី', 'manage_user.php')
		});

		$('.edit_user').click(function() {
			uni_modal('កែប្រែអ្នកប្រើប្រាស់', 'manage_user.php?id=' + $(this).attr('data-id'))
		});

		$('.delete_user').click(function() {
			_conf("តើអ្នកប្រាកដជាចង់លុបអ្នកប្រើប្រាស់នេះមែនទេ?", "delete_user", [$(this).attr('data-id')])
		});
	});
...
<style>
	...
	td center a.edit_user {
		background-color: #ffec3d;
	}

	/* Specific styles for the delete icon */
	td center a.delete_user {
		background-color: #f44336;
	}

	/* Specific styles for the new user icon */
	.new_user {
